...
function populateColorSuggestions() {
  const container = document
    .getElementById("colorSuggestions");
  container.innerHTML = "";
...
document.getElementById(
  ...
  function () {
    ...
    if(/^#[0-9A-Fa-f]{6}$/.test(
      ...
      localStorage.setItem(
        "appBackgroundColor", hex);
    } else {
      alert(
        "Hex invalide. Ex: #f0f0f0");
    }
    this.style.visibility = "hidden";
  });
...
function applyGradientBackground(
  colorInput) {
  const rgb = parseAnyColorToRGB(
    colorInput);
  if(!rgb) return;
  const lighten = (v, amt) => Math.min(
    255, Math.round(v + amt));
  const darken = (v, amt) => Math.max(0,
    Math.round(v - amt));
  const lightRGB =
    `rgb(${lighten(rgb.r, 60)}, ${lighten(rgb.g, 60)}, ${lighten(rgb.b, 60)})`;
  const darkRGB =
    `rgb(${darken(rgb.r, 40)}, ${darken(rgb.g, 40)}, ${darken(rgb.b, 40)})`;
  const baseRGB =
    `rgb(${rgb.r}, ${rgb.g}, ${rgb.b})`;
  document.body.style.background =
    `linear-gradient(to bottom, ${darkRGB}, ${baseRGB}, ${lightRGB})`;
  ...
  const authIcon = document
    .getElementById("authStatusIcon");
  if(authIcon) {
    authIcon.style.color = lightRGB
      .replace('rgb(', 'rgba(')
      .replace(')', ', 0.5)');
  }
}
...
function openAppTitleColorPicker() {
  ...
  openHexColorModal("title",
  savedColor);
}

function openHexColorModal(target,
  currentColor = "") {
  hexTarget = target;
...
function applyHexColor() {
  ...
  if(testEl.style.color === "") {
    alert(
      "Couleur invalide. Essayez un nom de couleur ou un code hex (#ff8800)."
      );
    return;
  }
  if(hexTarget === "title") {
    ...
    localStorage.setItem(
      "appTitleColor", color);
    showToast(
      "Couleur du titre modifiée !",
      "success");
  } else if(hexTarget ===
    "background") {
    applyGradientBackground(
    color); // ✅ handles both hex and named colors
    localStorage.setItem(
