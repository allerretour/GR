function promptEmojiChange(index) {
  const currentEmoji = shortcuts[index]
    .emoji || "";
  const recent = JSON.parse(localStorage
    .getItem("recentEmojis") || "[]");
...
createPickerContainer();
  ...
  if(recent.length > 0) {
    createRecentSection(picker, recent,
      currentEmoji, index, overlay);
  } else if(currentEmoji) {
    createCurrentEmojiSection(picker,
      currentEmoji);
  }
  createEmojiGrid(picker, index,
    overlay);
  createFooterButtons(picker, index,
    overlay);
  enableCloseOnOutsideClick(picker,
    overlay);
...
function createRecentSection(picker,
  recent, currentEmoji, index, overlay
  ) {
  const sectionTop = document
    .createElement("div");
  sectionTop.style.display = "flex";
  sectionTop.style.justifyContent =
    "space-between";
  sectionTop.style.alignItems =
  "center";
...
function createCurrentEmojiSection(
  picker, currentEmoji) {
...
function createEmojiGrid(picker, index,
  overlay) {
...
function createFooterButtons(picker,
  index, overlay) {
  const row = document.createElement(
    "div");
  row.style.display = "flex";
...
function enableCloseOnEscape(picker,
  overlay) {
...
function saveRecentEmoji(emoji) {
  let recent = JSON.parse(localStorage
    .getItem("recentEmojis") || "[]");
  recent = [emoji, ...recent.filter(e =>
    e !== emoji)].slice(0, 5);
