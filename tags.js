// tags.js
function toggleTags() {
  const tagContainer = document
    .getElementById("tagFilters");
  tagContainer.classList.toggle(
    "hidden");
...
function getTagColor(tag) {
  ...
  for(let i = 0; i < tag.length; i++) {
    hash = tag.charCodeAt(i) + ((hash <<
      5) - hash);
  }
  return tagcolors[Math.abs(hash) %
    tagcolors.length];
}
...
function displayTagFilters() {
  ...
  const tagSet = new Set();
  shortcuts.forEach(s => (s.tags || [])
    .forEach(t => tagSet.add(t)));
  if(tagSet.size === 0) return;
  const allTags = Array.from(tagSet);
  // Maintain tag order with new/removed tags
  tagOrder = tagOrder.filter(t =>
      allTags.includes(t))
    .concat(allTags.filter(t => !
      tagOrder.includes(t)));
  // Check if any favorites exist
  const hasFavorites = shortcuts.some(
    s => s.favorite);
  if(hasFavorites) {
    // "Favoris" button
    const favBtn = document
      .createElement("span");
    favBtn.className = "tag-filter" + (
      showOnlyFavorites ? " active" :
      "");
    favBtn.textContent = "⭐ Favoris";
    favBtn.title =
      "Afficher uniquement les raccourcis favoris";
    favBtn.onclick = () => {
      activeTagFilter = [];
      ...
      btn.ondrop = e => {
        ...
        const fromTag = e
          .dataTransfer.getData(
            "text/plain");
        const toTag = tag;
        const fromIndex = tagOrder
          .indexOf(fromTag);
        const toIndex = tagOrder
          .indexOf(toTag);
        if(fromIndex > -1 &&
          toIndex > -1 &&
          fromIndex !== toIndex) {
