...
function mergeShortcuts(event) {
  ...
  if (!file) return;
  if (
    !confirm("Souhaitez-vous fusionner cette liste avec la liste actuelle ?")
  ) {
    return;
  }
...
function exportVisibleShortcuts(format) {
  ...
  const listTitle =
    document.getElementById("appTitle").textContent.trim() || "Sans titre";
  ...
  if (format === "lst" || format === "both") {
    ...
    const jsonBlob = new Blob([JSON.stringify(jsonData, null, 4)], {
  ...
  showToast("⬆️ Exportation réussie !", "success");
}
  ...
  reader.onload = function (e) {
    try {
      ...
      if (Array.isArray(importedData)) {
        ...
        Array.isArray(importedData.shortcuts)
      ) {
        ...
        if (importedData.appBackgroundColor) {
          localStorage.setItem(
            "appBackgroundColor",
            importedData.appBackgroundColor,
          );
      ...
      showToast("⬇️ Chargement de la liste réussie !", "success");
    } catch {
      alert(
