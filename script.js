...
document.getElementById("authStatusIcon").addEventListener("click", () => {
  if (!GLOBAL_PASSWORD) {
    ...
    const newPwd = prompt("🔐 Définir un mot de passe :");
    if (newPwd && newPwd.trim().length > 0) {
      localStorage.setItem("globalPassword", newPwd.trim());
      GLOBAL_PASSWORD = newPwd.trim();
      showToast("🔒 Mot de passe défini. Accès sécurisé activé.", "success");
      updateAuthStatusIcon();
    } else if (newPwd !== null) {
      showToast("⚠️ Mot de passe invalide.", "error");
    }
    return;
  }
...
function openVisibleShortcutsInTabs() {
  const visibleCards = Array.from(
    document.querySelectorAll("#shortcuts .shortcut"),
  ).filter((card) => card.offsetParent !== null);
  const urls = visibleCards
    .map((card) => {
      const index = parseInt(card.getAttribute("data-index"));
      const shortcut = shortcuts[index];
      if (!shortcut || !shortcut.url || shortcut.url.trim() === "?")
        return null;
  ...
  if (!confirm(`⚠️ Ouvrir ${urls.length} lien(s) dans de nouveaux onglets ?`))
    return;
  const MAX_TABS = 10;
...
function toggleSorting() {
  ...
  document.getElementById("sortStatus").textContent = alphabeticalSorting
    ? "Tri: Alphabétique"
    : "Tri: Manuel";
  displayShortcuts();
}
...
function displayShortcuts() {
    ...
    list = list.filter((shortcut) => {
      return (
        ...
        (shortcut.tooltipPlain &&
          shortcut.tooltipPlain.toLowerCase().includes(searchTerm)) ||
  ...
  document.getElementById("filterModeLabel").textContent = isAndMode
    ? "ET"
    : "OU";
  if (list.length === 0) {
  ...
  list.forEach((shortcut) => {
    ...
    shortcutElement.setAttribute(
      "title",
      escapeHTML(`${baseText}${tooltipContent}`),
    );
      ...
      touchHoldTimer = setTimeout(() => {
        ...
        const base =
          url === "?"
            ? ""
            : `<a href="${url}" target="_blank" rel="noopener noreferrer">Lien du raccourci</a>`;
        const tooltip = shortcut.tooltip?.trim()
          ? `<br>${shortcut.tooltip}`
          : "";
        showTooltipModal(`${base}${tooltip}`, true, shortcut.name);
...
function handleLogoUpload(event, side) {
  ...
  reader.onload = function (e) {
    ...
    const toastMessage =
      side === "left" ? "Logo gauche modifié !" : "Logo droit modifié !";
...
document.addEventListener("DOMContentLoaded", () => {
  document
    .getElementById("leftLogoInput")
    .addEventListener("change", (e) => handleLogoUpload(e, "left"));
  document
    .getElementById("rightLogoInput")
    .addEventListener("change", (e) => handleLogoUpload(e, "right"));
  loadUIState();
  // Search Bar
  document.getElementById("searchContainer").style.display =
    uiToggleState.searchBar ? "flex" : "none";
  // Tag Filters
  document
    .getElementById("tagFilters")
    .classList.toggle("hidden", !uiToggleState.tagFilters);
        ...
        handlers: {
          image: function () {
            ...
            input.onchange = async () => {
              ...
              const resizedBase64 = await resizeAndCompressImage(
                file,
                150,
                50 * 1024,
              ); // 150px, 50KB
              if (!resizedBase64) {
                alert(
                  "❌ Image trop grande même après compression (max 150px et 50Ko).",
                );
                return;
              }
