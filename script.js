...
let hexTarget = null;
const storedFavoriteMode = localStorage
  .getItem("showOnlyFavorites");
let showOnlyFavorites =
  storedFavoriteMode ===
  "false"; // ✅ restore as boolean
const icons = Quill.import('ui/icons');
icons['hr'] =
  '<span style="display:inline-block;width:100%;border-top:1px solid #888;margin-top:2px;"></span>';

function updateAuthStatusIcon() {
  const icon = document.querySelector(
    "#authStatusIcon i");
  const container = document
    .getElementById("authStatusIcon");
  if(!icon || !container) return;
  if(!GLOBAL_PASSWORD) {
    icon.className = "fas fa-expand";
    container.title =
      "🔓 Accès libre (non sécurisé)";
  } else if(sessionStorage.getItem(
      "authenticated") === "true") {
    icon.className = "fas fa-lock-open";
    container.title =
      "🔓 Authentifié (sécurisé)";
  } else {
    icon.className = "fas fa-lock";
    container.title =
      "🔒 Protégé par mot de passe (non authentifié)";
  }
}
document.getElementById(
  ...
  .addEventListener("click", () => {
    if(!GLOBAL_PASSWORD)
  return; // No password, nothing to do
    ...
    if(isAuthenticated) {
      ...
      if(confirm(
          "🔒 Voulez-vous verrouiller l'accès ?"
          )) {
...
function ensureAuthenticated() {
  ...
  const input = prompt(
    "🔒 Entrez le mot de passe pour accéder à cette fonction :"
    );
  if(input === GLOBAL_PASSWORD) {
    sessionStorage.setItem(
      "authenticated", "true");
...
const titleLockBtn = document
  .getElementById("titleLockBtn");
const lockIcon = titleLockBtn
  .querySelector("i");
...
titleLockBtn.addEventListener("click",
  toggleTitleLock);
...
window.addEventListener(
  "DOMContentLoaded", () => {
    GLOBAL_PASSWORD = localStorage
      .getItem("globalPassword") ||
      "";
    updateAuthStatusIcon();
    const saved = localStorage
      .getItem("appTitleColor");
    if(saved) document.getElementById(
        "appTitle")
      .style.color = saved;
  });
async function pasteFromClipboard(
  ...
  try {
    ...
    alert(
      "⚠️ Autorisation refusée. Utilisez Ctrl+V pour coller manuellement."
      );
    console.warn(
...
function openVisibleShortcutsInTabs() {
  ...
  if(urls.length === 0) {
    showToast(
      "Aucun lien valide à ouvrir.");
    return;
  }
  if(!confirm(
      `⚠️ Ouvrir ${urls.length} lien(s) dans de nouveaux onglets ?`
      )) return;
  const MAX_TABS = 10;
  const slice = urls.slice(0, MAX_TABS);
  if(urls.length > MAX_TABS) {
    alert(
      `Seuls les ${MAX_TABS} premiers liens seront ouverts pour éviter les blocages navigateur.`
      );
  }
  slice.forEach(url => window.open(url,
    "_blank"));
}
...
function saveUIState() {
  localStorage.setItem(UI_STATE_KEY,
    JSON.stringify(uiToggleState));
}
...
function ensureDefaultShortcut() {
  const isFreshInstall = !Array.isArray(
  ...
  if(isFreshInstall) {
    ...
    localStorage.setItem("manualOrder",
      JSON.stringify(manualOrder));
    uiToggleState = {
      ...DEFAULT_UI_TOGGLE_STATE
    };
    saveUIState();
    compactMode = false;
    localStorage.setItem("compactMode",
      "false");
    activeTagFilter = [];
    saveActiveTagFilter();
...
function resetLogos() {
  ...
  const leftImg = document
    .getElementById("leftLogo");
  const rightImg = document
    .getElementById("rightLogo");
  if(leftImg) leftImg.style.display =
    "none";
  if(rightImg) rightImg.style.display =
    "none";
  setExportNeeded(
  true); // Mark state as changed
...
function updateExportStatusDot() {
  const dot = document.getElementById(
    'exportStatusDot');
  dot.textContent = isExportNeeded ?
    'modifié' : 'non-modifié';
  dot.className = 'status-dot ' + (
    isExportNeeded ? 'modifie' :
    'non-modifie');
}

function updateLastExportDisplay() {
  const lastExport = localStorage
    .getItem("lastExportFilename");
  const label = document.getElementById(
    "lastExport");
  label.textContent = lastExport ?
    `Dernier export : ${lastExport}` :
    "Dernier export : aucun";
}
...
function toggleSearchBar() {
  ...
  const isHidden = searchContainer.style
    .display === "none" ||
  ...
  const el = document.getElementById(
    "searchContainer");
  uiToggleState.searchBar = !
    uiToggleState.searchBar;
  el.style.display = uiToggleState
    .searchBar ? "flex" : "none";
  saveUIState();
}
...
function loadShortcuts() {
  const savedData = localStorage
    .getItem("shortcuts");
  const savedOrder = localStorage
    .getItem("manualOrder");
  const savedTagOrder = localStorage
    .getItem("tagOrder");
  tagOrder = savedTagOrder ? JSON.parse(
    savedTagOrder) : [];
  shortcuts = savedData ? JSON.parse(
    savedData) : [];
  manualOrder = savedOrder ? JSON.parse(
    savedOrder) : [...shortcuts];
  // ✅ MIGRATION: Add tooltipPlain if missing
  let updated = false;
  shortcuts.forEach(sc => {
    if(!sc.tooltipPlain && sc
      .tooltip) {
      const temp = document
        .createElement("div");
      temp.innerHTML = sc.tooltip;
      sc.tooltipPlain = temp
        .textContent.trim();
      updated = true;
    }
  ...
  if(updated) {
    saveShortcuts
  (); // Resave only if we made changes
  }
  ensureDefaultShortcut
...
function toggleAddSection() {
  ...
  const isHidden = section.style
    .display === "none";
  section.style.display = isHidden ?
    "block" : "none";
  if(isHidden) {
    setTimeout(() => {
      document.getElementById(
...
function toggleSorting() {
  ...
  document.getElementById("sortStatus")
    .textContent = alphabeticalSorting ?
    "Tri: Alphabétique" : "Tri: Manuel";
  displayShortcuts();
}

function toggleEditMode() {
  editMode = document.getElementById(
      "editToggle")
    .checked;
...
function displayShortcuts() {
  ...
  if(showOnlyFavorites && !shortcuts
    .some(s => s.favorite)) {
    showOnlyFavorites = false;
    localStorage.setItem(
      "showOnlyFavorites", false);
    showToast(
      "Aucun favori — retour à Tous");
  }
  const container = document
  ...
  if(activeTagFilter.length > 0) {
    const useAndMode = document
      .getElementById(
        "tagFilterModeToggle")
      .checked;
    list = list.filter(shortcut => {
      const shortcutTags = shortcut
        .tags || [];
      return useAndMode ?
        activeTagFilter.every(tag =>
          shortcutTags.includes(tag)
          ) : activeTagFilter.some(
          tag => shortcutTags
          .includes(tag));
    });
  }
  ...
  const isAndMode = document
    .getElementById(
      "tagFilterModeToggle")
    .checked;
  document.getElementById(
      "filterModeLabel")
    .textContent = isAndMode ? "ET" :
    "OU";
  if(list.length === 0) {
    const emptyMessage = document
      .createElement("div");
    emptyMessage.textContent =
      "😢 Aucun raccourci à afficher.";
    ...
    document.getElementById(
        "shortcutCount")
      .textContent =
      `Affichés: 0 / Total: ${shortcuts.length}`;
    saveActiveTagFilter();
    displayTagFilters();
    return;
  }
  document.querySelectorAll(
      ".tag-filter")
    .forEach(el => {
      el.classList.remove("and-mode",
        "or-mode");
      el.classList.add(isAndMode ?
        "and-mode" : "or-mode");
    });
  list.forEach((shortcut) => {
    const trueIndex = shortcuts
      .indexOf(shortcut);
    const tagsHTML = (shortcut
        .tags || [])
      .sort((a, b) => a
        .localeCompare(b))
      .map(tag =>
        `<span class="tag" style="background-color:${getTagColor(tag)}">${escapeHTML(tag)}</span>`
        )
      .join(" ");
    const shortcutElement = document
      .createElement("div");
    shortcutElement.style.position =
      "relative";
    const trimmedUrl = shortcut.url
      .trim();
    const isInfoOnly =
      trimmedUrl === "?";
    const isFileUrl = trimmedUrl
      .toLowerCase()
      .startsWith("file://");
    const nameColor = isInfoOnly ?
      "#0079fa" : "inherit";
    const namePrefix = isInfoOnly ?
      '<i class="fa-solid fa-lightbulb" style=" margin-right: 2px;color: gold;"></i>' :
      "";
    shortcutElement.className =
      "shortcut" + (compactMode ?
        " compact" : "") + (
        editMode ? " editmode" : ""
        );
    const baseText = isInfoOnly ?
    ...
    shortcutElement.setAttribute(
      "title", escapeHTML(
        `${baseText}${tooltipContent}`
        ));
    shortcutElement.setAttribute(
      "data-index", trueIndex);
    shortcutElement.style.cursor =
      editMode ? 'default' :
      'pointer';
    if(!alphabeticalSorting &&
      editMode) {
    ...
    shortcutElement
      .addEventListener(
        "touchstart", (e) => {
          if(editMode || e.touches
            .length !== 1) return;
          // Prevent default touch interactions
          shortcutElement.style
            .userSelect = "none";
          shortcutElement.style
            .webkitUserSelect =
            "none";
...
`;
    if(!editMode) {
      ...
      favBtn.className =
        "favorite-toggle";
      favBtn.innerHTML = shortcut
        .favorite ? "⭐" : "☆";
      favBtn.title = shortcut
        .favorite ? "Favori" :
        "Marquer comme favori";
      favBtn.style.position =
        "absolute";
      favBtn.style.top = "6px";
      favBtn.style.right = "8px";
      favBtn.style.fontSize =
      "20px";
      favBtn.style.cursor =
        "pointer";
      favBtn.style.zIndex = "2";
      favBtn.style.color = shortcut
        .favorite ? "gold" :
        "#aaa"; // ✅ Gold or grey
      favBtn.style.textShadow =
        "none"; // ✅ No glow
      favBtn.onclick = (e) => {
        e
      .stopPropagation(); // ✅ prevent opening the URL
  ...
  saveActiveTagFilter
(); // ✅ Save filter every time it’s applied
  displayTagFilters();
  document.getElementById(
      "shortcutCount")
    .textContent =
...
let tagOrder = JSON.parse(localStorage
  .getItem("tagOrder")) || [];

function saveTagOrder(order) {
  localStorage.setItem("tagOrder", JSON
    .stringify(order));
  setExportNeeded(true);
}
...
function deleteShortcut(index) {
  ...
  const confirmMsg =
    `Supprimer le raccourci "${shortcut.name}" ?`;
...
function showTooltipModal(text, isHtml =
  false, shortcutName = "") {
  const tooltipContent = document
    .getElementById("tooltipContent");
  const safeText = text?.trim() ||
    "Aucune info disponible.";
  let finalContent = safeText;
  if(shortcutName) {
    const titleHtml =
      `<div style="color: #222; text-align: center; font-size: 1.3rem;">💡 Info du raccourci : <strong>${escapeHTML(shortcutName)}</strong></div>
<hr style="border: none; border-top: 1px solid #ccc;" />`;
    finalContent = titleHtml +
      finalContent;
  }
  tooltipContent.innerHTML = isHtml ?
    finalContent : escapeHTML(
      finalContent);
  // Ajout des boutons "copier" pour chaque lien
  const links = tooltipContent
  ...
  links.forEach(linkEl => {
    ...
    wrapper.className =
      "link-with-copy";
    linkEl.parentNode.insertBefore(
      wrapper, linkEl);
...
function confirmAdd() {
  const tooltipHtml = quill.root
    .innerHTML.trim();
  const tooltipText = quill.getText()
  ...
  if(name && url) {
    ...
    alert(
      "Veuillez remplir au moins le nom et l'URL."
      );
  }
}
...
function editShortcut(index) {
  ...
  document.getElementById(
    ...
    .value = '';
  // Prepare tags
  selectedTags = [...(shortcut
  .tags || [])];
  populateTagSuggestions
(); // Get updated list of all tags
  renderSelectedTags
(); // Display current tags in chip format
  // Set index
  editIndex = index;
...
function confirmEdit() {
  ...
  if(name && url && editIndex !==
    ...
    shortcuts[editIndex] = {
      ...
      tooltipPlain: tooltipText,
      info,
      emoji: shortcuts[editIndex]
        .emoji || DEFAULT_EMOJI,
      favorite: shortcuts[editIndex]
        .favorite ||
        false // ✅ preserve or fallback
    };
    saveShortcuts();
    displayShortcuts();
    closeEditModal();
...
function handleLogoUpload(event, side) {
  ...
  if(file.size > MAX_SIZE) {
    alert(
      "❌ L’image dépasse 300 Ko. Veuillez en choisir une plus petite."
      );
    return;
  }
  ...
  reader.onload = function (e) {
    ...
    const key = side === "left" ?
      "leftLogo" : "rightLogo";
    const imgId = side === "left" ?
      "leftLogo" : "rightLogo";
    const toastMessage = side ===
      "left" ?
      "Logo gauche modifié !" :
      "Logo droit modifié !";
...
function loadLogos() {
  const left = localStorage.getItem(
    "leftLogo");
  const right = localStorage.getItem(
    "rightLogo");
  const leftImg = document
    .getElementById("leftLogo");
  const rightImg = document
...
function clearShortcuts() {
  ...
  (); // ✅ Add fallback examples
    ...
    localStorage.removeItem(
    "rightLogo");
    const leftLogo = document
      .getElementById("leftLogo");
    const rightLogo = document
      .getElementById("rightLogo");
    if(leftLogo) leftLogo.style
      .display = "none";
    if(rightLogo) rightLogo.style
      .display = "none";
    // 📝 Update display
    setExportNeeded(true);
    displayShortcuts();
...
function drop(event) {
  ...
  const toIndex = parseInt(target
    .dataset.index);
  if(fromIndex !== toIndex) {
    const movedItem = shortcuts.splice(
      fromIndex, 1)[0];
    shortcuts.splice(toIndex, 0,
      movedItem);
    saveShortcuts();
    displayShortcuts();
  }
...
document.addEventListener(
  "DOMContentLoaded", () => {
    ...
    document.getElementById(
        "searchContainer")
      .style.display = uiToggleState
      .searchBar ? "flex" : "none";
    // Tag Filters
    document.getElementById(
        "tagFilters")
      .classList.toggle("hidden", !
        uiToggleState.tagFilters);
    const savedColor = localStorage
      .getItem("appBackgroundColor");
    if(savedColor) {
      applyGradientBackground(
        savedColor);
    }
...
window.onload = function () {
  ...
  const savedCompact = localStorage
    .getItem(
    "compactMode"); // ✅ Restore compact
  compactMode = savedCompact ===
    "true";
  showOnlyFavorites = localStorage
    .getItem("showOnlyFavorites") ===
    "true";
  const savedFilter = localStorage
    .getItem("activeTagFilter");
  if(savedFilter) {
    try {
      ...
      console.warn(
        "Erreur parsing activeTagFilter:",
        e);
    }
  }
  loadShortcuts();
  const savedTitle = localStorage
    .getItem("appTitle");
  if(savedTitle) {
    document.getElementById(
        "appTitle")
      .textContent = savedTitle;
  }
